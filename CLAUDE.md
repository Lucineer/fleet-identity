# CLAUDE.md — Fleet Identity

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Fleet Identity
- **Role**: Fleet Identity
- **URL**: https://fleet-identity.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/fleet-identity
- **Branch**: master
...
### Deploy
```bash
cd /tmp/fleet-identity && wrangler deploy
```

### Health Check
```bash
curl -s https://fleet-identity.casey-digennaro.workers.dev/health
curl -s https://fleet-identity.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/identity | API endpoint |
| /api/priorities | API endpoint |
| /api/priority | API endpoint |
| /api/resolve | API endpoint |
| /api/snapshot | API endpoint |
| /api/snapshots | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=fleet-identity
...
- **vessel.json capabilities**: self-awareness, priority-management, conflict-resolution
...
1. Keep the vessel small. ~162 lines is the current size.
